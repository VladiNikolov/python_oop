from project.additional_methods.food_factory import FoodFactory
from project.additional_methods.drink_factory import DrinkFactory
from project.additional_methods.table_factory import TableFactory
...
class Bakery:
    ...
    def order_drink(self, table_number: int, *drink_names: []):
        ...
        skipped_order_drink = f"{self.name} does not have in the menu:\n"

        for drink_name in drink_names:
            drink = self.find_drink_by_name(drink_name)

            if drink is None:
                skipped_order_drink += drink_name + "\n"
            else:
                table.order_drink(drink)
                ordered_drink += str(drink) + "\n"

        return ordered_drink.strip() + "\n" + skipped_order_drink.strip()
